...
Phase 1 (the original stateless-proxy design, since superseded and removed)
...
written when phase 2 implementation starts.
...
- **Normalized, not a JSON blob.** Matches how CP itself models this domain —
  ...
  hearing.
- **Immutable version rows.** Each `cp_version` row is one defendant's PCR
...
- **Surrogate row identity, source-minted business identity.**
  `cp_version_pk` (a UUID generated by this service at write time) is the
  table's primary key. `cp_version.event_id` carries the v2 §7
  ...
  **not** the primary key. The source-id-propagation pipeline (v2 §7, §13
  ...
  older rows) is the standard fix — see §3's `cp_version` section for the
  full column layout.
- **Polymorphic children, not source-specific branching.** `cp_offence` and
  `cp_judicial_result` each hang off *either* of two parents, matching v2
...
erDiagram
    ...
    CP_VERSION {
        uuid cp_version_pk PK "surrogate, generated here — see §3"
        string event_id "nullable — v2 §7's source-propagated id; NOT the PK, see §3"
        uuid defendant_id
        uuid case_hearing_id FK
    ...
    CP_OFFENCE {
        ...
        uuid version_pk FK "nullable — set only when parent is a version; FK to cp_version.cp_version_pk"
    ...
    CP_COURT_APPLICATION {
        uuid id PK "CP's own application id"
        uuid version_pk FK "FK to cp_version.cp_version_pk"
        string reference
        string type
        string decision
        date decision_date
    ...
    CP_CASE_HEARING ||--o{ CP_CASE_MARKER          : "has (case-level)"
    CP_CASE_HEARING ||--o{ CP_VERSION              : "one per defendant"
    CP_VERSION      ||--o{ CP_OFFENCE              : "case-level (direct)"
...
`CP_OFFENCE` and `CP_JUDICIAL_RESULT` each show two parent-FK edges in the
...
- **Owns:** the hearing-level facts from `HearingDetails` (minus
  `nextHearing` — see `cp_version` below) — `court_house_code`,
...
- **No `overall_conviction_date` column** — the hearing-level aggregate
  ...
  separately; `conviction_date` lives only on `cp_offence`.

### `cp_case_marker`

Case-level markers (e.g. `DomesticViolence`), child of `cp_case_hearing`
...
- **Key:** `cp_version_pk`, a surrogate UUID generated by this service at
  write time.
- **`event_id`:** the v2 §7 source-propagated id — nullable, not part of
  the PK. Populated once the cross-team propagation pipeline delivers it;
  rows written before then simply have `event_id = NULL`, same as
  `PcrVersion.id` is `null` today in phase 1. A partial unique constraint,
  `UNIQUE (event_id, defendant_id) WHERE event_id IS NOT NULL`, still
...
- **Defendant identity fields:** `master_defendant_id`, embedded directly
  on this table rather than a separate `cp_defendant` table, because it's
...
Polymorphic child — parented by **either** `cp_version` (a defendant's own
case-level offences, direct) **or** `cp_court_application` (offences linked
...
- **Parent FKs (exactly one set, enforced by `CHECK`):** `version_pk`
  ...
  the surrogate `cp_version_pk` is already globally unique per row, so no
...
Bail applications, variations, etc. — child of `cp_version` (many per
version).
...
- **Coverage note:** because `cp_offence` is itself polymorphic, this one
  table already covers all four of CP's real result sources — a result on a
  `cp_offence` row parented by `cp_court_application` is exactly a
...
- **`cp_version` is the retention anchor.** Each row carries its own
  `expires_at`; purge is a TTL sweep, not an event-driven delete.
- **Cascade on `cp_version` deletion:** all four child tables —
  `cp_offence`, `cp_court_application`, `cp_judicial_result`,
  `cp_judicial_result_prompt` — cascade, including both `cp_offence`
  parentage paths (direct and via `cp_court_application`).
- **`cp_case_hearing` is swept separately, not cascaded.** Its lifetime
  ...
  secondary sweep deletes a `cp_case_hearing` row only once it has zero
  remaining `cp_version` children — this is the sole deletion mechanism
  for this table.
- **`cp_case_marker` cascades with its `cp_case_hearing` parent** — it has
...
- **Id shape not locked (v2 §7, §13 item 5):** ULID (recommended) vs
  UUID+`sharedResultTime`. This changes `cp_version.event_id`'s column
  type and whether a separate ordering column is needed alongside it — it
  does not affect the PK, since `cp_version_pk` (§3) is the surrogate key
  and `event_id` is a plain nullable column.
...
implements against `cp_version` (fetch latest by
`(case_hearing_id, defendant_id)` + recency, or fetch a specific version by
`(event_id, defendant_id)` once that's implemented) reads against the same
schema either way.
...
Migration files themselves (`db/migration/V1.NNN__*.sql`, the actual
...
migration tooling, not the DDL. Naming follows the `V<major>.<NNN>__<description>.sql`
convention already established in this org's other Flyway-backed services
(`service-cp-crime-hearing-results-document-subscription`, the
`postgres-encrypt-demo`/`postgres-lock`/`postgres-springboot4` demos) — not
flat `V1`/`V2`/... — so a future schema-breaking major version bump has
room to insert without renumbering everything already shipped.