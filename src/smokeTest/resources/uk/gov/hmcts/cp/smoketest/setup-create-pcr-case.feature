Feature: drive a real hearing result through the CP backend for PCR smoke testing
  ...
  Scenario: submit SPI-IN case creation, share a custodial result, record the resolved ids
    ...
    * def soapEnvelope = soapTemplate.replaceAll('REQUEST_ID', requestId).replace('NEW_URN', caseUrn).replace('TODAY+4', hearingDate).replace('ADDRESS_LINE1', '1 Smoke Test Street')
    ...
    * def responseText = new java.lang.String(responseBytes, 'UTF-8')
    ...
    # Retry budget for the resolution polls below - CP's query APIs are eventually consistent.
    ...
    * def draftPayload = draftPayload.replaceAll('TIMP_LINE_ID', resultLineIds.timp).replaceAll('IMP_LINE_ID', resultLineIds.imp).replaceAll('VULNERABILITY_LINE_ID', resultLineIds.vulnerability).replaceAll('VSA_LINE_ID', resultLineIds.vsa).replaceAll('OATC_LINE_ID', resultLineIds.oatc).replaceAll('FVS_LINE_ID', resultLineIds.fvs).replaceAll('NOCOLLO_LINE_ID', resultLineIds.nocollo).replaceAll('ACON_LINE_ID', resultLineIds.acon).replaceAll('COLLOM_LINE_ID', resultLineIds.collom).replaceAll('PAYT_LINE_ID', resultLineIds.payt).replaceAll('PDATE_LINE_ID', resultLineIds.pdate)
    * def draftPayload = draftPayload.replaceAll('HEARING_ID', hearingId).replaceAll('HEARING_DAY', hearingDay).replaceAll('ORDER_DATE', orderDate).replaceAll('DEFENDANT_ID', defendantId).replaceAll('CASE_ID', caseId).replaceAll('OFFENCE_ID', offenceId).replaceAll('FUTURE_DATE', futureDate)
    ...
    * def sharedPayload = sharedPayload.replaceAll('TIMP_LINE_ID', resultLineIds.timp).replaceAll('IMP_LINE_ID', resultLineIds.imp).replaceAll('VSA_LINE_ID', resultLineIds.vsa).replaceAll('FVS_LINE_ID', resultLineIds.fvs).replaceAll('COLLOM_LINE_ID', resultLineIds.collom).replaceAll('PAYT_LINE_ID', resultLineIds.payt).replaceAll('PDATE_LINE_ID', resultLineIds.pdate)
    * def sharedPayload = sharedPayload.replaceAll('ORDER_DATE', orderDate).replaceAll('SHARED_DATE', todayDate).replaceAll('DEFENDANT_ID', defendantId).replaceAll('CASE_ID', caseId).replaceAll('OFFENCE_ID', offenceId).replaceAll('FUTURE_DATE', futureDate)
